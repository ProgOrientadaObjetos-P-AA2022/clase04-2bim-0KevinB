    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < edad.length; i++) {
            ...
            if (edad[i] >= 0 && edad[i] <= 18) {
                PasajeMenorEdad e = new PasajeMenorEdad(pasajeFijo);
                e.establecerPersona(p);
                pasajes.add(e);
            } else {
                if (edad[i] > 18 && edad[i] <= 25) {
                    PasajeUniversitario e = new PasajeUniversitario(pasajeFijo);
                    e.establecerPersona(p);
                    pasajes.add(e);
                } else {
                    if (edad[i] > 25 && edad[i] < 65) {
                        PasajeNormal e = new PasajeNormal(pasajeFijo);
                        e.establecerPersona(p);
                        pasajes.add(e);
                    } else {
                        if (edad[i] >= 65) {
                            PasajeTerceraEdad e = new PasajeTerceraEdad(pasajeFijo);
                            e.establecerPersona(p);
                            pasajes.add(e);
                        }
                    }
